...
locals {
  ...
  ecr_names = toset(["dashboard-repo", "authors-repo", "books-repo"])
}
...
resource "aws_ecr_repository" "app_repos" {
  ...
  tags = merge(local.common_tags, { 
    Name = "ecr-${each.key}-${local.env_suffix}" 
  })
}
...
#   for_each                 = toset(["books", "authors", "dashboard"])
#   family                   = "lirw-task-${local.env_suffix}"
#   # network_mode             = "host"
#   # network_mode             = "bridge"
#   network_mode             = "awsvpc"
...
resource "aws_ecs_service" "app_service" {
  ...
  load_balancer {
    ...
    target_group_arn = aws_lb_target_group.app_tg[each.key].arn
    container_name   = "lirw-${each.key}" # Matches the name in your JSON template
